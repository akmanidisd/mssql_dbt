...
WHERE COLUMN_NAME ILIKE '%_BW'
  AND TABLE_NAME NOT IN ('USER_CONTACT_PREF_FACT','USER_ACCOUNT_ACTION') -- PK WITH 2 COLUMNS
...
;
-- SELECT * FROM MS_RAW.STG_META.SF_COLUMNS WHERE TABLE_NAME = 'USER_ACCOUNT_FACT' ORDER BY ORDINAL_POSITION;
...
--   AND NEW_COLUMN_NAME = REPLACE(COLUMN_NAME,'_BW','_ID')   -- 16
ORDER BY TABLE_SCHEMA, TABLE_NAME, COLUMN_NAME, BASE_COLUMN_NAME, NEW_COLUMN_NAME
;
