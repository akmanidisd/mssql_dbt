...
INSERT INTO MS_RAW.STG_META.SF_COLUMNS
      (TABLE_SCHEMA, TABLE_NAME, NEW_TABLE_NAME, COLUMN_NAME, BASE_COLUMN_NAME, NEW_COLUMN_NAME,             NEW_COLUMN_TYPE, DATA_TYPE)  
SELECT TABLE_SCHEMA, TABLE_NAME, NEW_TABLE_NAME, COLUMN_NAME, BASE_COLUMN_NAME, BASE_COLUMN_NAME || '_DATE', 'DATE',          DATA_TYPE  
FROM MS_RAW.STG_META.SF_COLUMNS C
WHERE DATA_TYPE = 'TIMESTAMP_TZ'
  AND NEW_COLUMN_TYPE = 'TIMESTAMP_NTZ'
  AND NOT EXISTS (SELECT 1 
                  FROM MS_RAW.STG_META.SF_COLUMNS X
                  WHERE X.TABLE_SCHEMA = C.TABLE_SCHEMA
                    AND X.TABLE_NAME   = C.TABLE_NAME
                    AND X.COLUMN_NAME  = C.COLUMN_NAME
                    AND X.DATA_TYPE    ='TIMESTAMP_TZ'
                    AND X.NEW_COLUMN_TYPE ='DATE'
                  )
