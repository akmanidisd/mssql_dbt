!!!!!!!!!!!!!!!!!!!!!! TO UPDATE ALL THE NEW_COLUMN_NAME FOR ALL THE SF_COLUMNS
FOR ALL SF_COLUMNS = PRIMARY_KEY_NAME AND PRIMARY_KEY_NAME != NEW_PRIMARY_KEY_NAME
UPDATE NEW_COLUMN_NAME = NEW_PRIMARY_KEY_NAME
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1
...
CREATE OR REPLACE TRANSIENT TABLE MS_RAW.STG_META.T_PRIMARY_KEYS AS
    SELECT
        ...
        LISTAGG(sfc.SF_COLUMN_NAME,',') OVER (PARTITION BY kcu.CONSTRAINT_SCHEMA, kcu.CONSTRAINT_NAME) AS PK_COLUMN_NAMES,
...
------------------------- PK_COLUMNS = 2 <START> ----------------------------------
    ...
    WITH TWO_COLUMNS AS (
        SELECT SF_TABLE_SCHEMA, 
               SF_TABLE_NAME, 
               FIRST_VALUE(SF_COLUMN_NAME)  
                    OVER (PARTITION BY SF_TABLE_SCHEMA, SF_TABLE_NAME ORDER BY ORDINAL_POSITION) 
                                                                 AS KEY_PART_1,
               LAST_VALUE(SF_COLUMN_NAME)  
                    OVER (PARTITION BY SF_TABLE_SCHEMA, SF_TABLE_NAME ORDER BY ORDINAL_POSITION) 
                                                                 AS KEY_PART_2,
               CASE WHEN SF_TABLE_NAME IN ('USER_CONTACT_PREF_FACT','USER_ACCOUNT_ACTION')
    ...
    UPDATE MS_RAW.STG_META.SF_TABLES
     ...
     WHERE SF_TABLE_NAME   = 'RATE_CARD_TIER'
    ;
...
------------------------- PK_COLUMNS = 2 <END> ----------------------------------
    ...
    -- 219 PKEYS, 219 PK Columns -> PK = TABLE+_ID
    SELECT count(distinct CONSTRAINT_SCHEMA, CONSTRAINT_NAME) as TOTAL_PKEYS
         , count(*) AS TOTAL_COLUMNS
...
-- INSERT THE NEW COLUMNS IN SF_COLUMNS >>>>>>>>>>>>>>>>>>>>>>>>>>>>>
-- STATS FROM PK IN SF_TABLES where sft.PK_COLUMNS = 2 
SELECT COUNT(*), -- 0
       COUNT(CASE WHEN sft.NEW_TABLE_NAME || '_ID'  = sft.NEW_PRIMARY_KEY_NAME                         THEN 1 END) AS MASTER_PKS,    -- 21
FROM MS_RAW.STG_META.SF_TABLES sft
LEFT JOIN MS_RAW.STG_META.SF_COLUMNS sfc
   ON sft.SF_TABLE_SCHEMA  = sfc.SF_TABLE_SCHEMA
  AND sft.SF_TABLE_NAME    = sfc.SF_TABLE_NAME
  AND sft.PRIMARY_KEY_NAME = sfc.SF_COLUMN_NAME
WHERE sft.PK_COLUMNS = 2 
  AND sfc.SF_COLUMN_NAME IS NULL
;
...
SELECT SF_TABLE_SCHEMA, SF_TABLE_NAME, NEW_PRIMARY_KEY_NAME, 
       NEW_PRIMARY_KEY_NAME, 'NUMERIC(18,0)', NEW_PRIMARY_KEY_EXPRESSION
FROM MS_RAW.STG_META.SF_TABLES
WHERE PK_COLUMNS = 2
  AND (SF_TABLE_SCHEMA, SF_TABLE_NAME, NEW_PRIMARY_KEY_NAME)
