...
-- 6 
UPDATE MS_RAW.STG_META.T_PRIMARY_KEYS
   SET NEW_COLUMN_NAME = NEW_TABLE_NAME_ID
...
-- 299 TABLES, 277 ONE_PK_COLUMN, 22 TWO_PK_COLUMNS, 321 COLUMNS
...
-- 233 MASTER PK, 215 , 18
SELECT COLUMN_NAME, TABLE_NAME, NEW_TABLE_NAME, NEW_COLUMN_NAME
  FROM MS_RAW.STG_META.T_PRIMARY_KEYS 
 WHERE PK_COLUMNS = 1 
   AND NEW_COLUMN_NAME = NEW_TABLE_NAME_ID
--   AND NEW_COLUMN_NAME  = COLUMN_NAME -- 215
--   AND NEW_COLUMN_NAME != COLUMN_NAME --  18
ORDER BY ALL;


-- 41 PK ARE SAME AS MASTER PK
SELECT MK.NEW_COLUMN_NAME AS MK_NEW_COLUMN_NAME, PK.TABLE_SCHEMA,  PK.TABLE_NAME, PK.COLUMN_NAME, MK.TABLE_NAME, MK.COLUMN_NAME, PK.*
FROM MS_RAW.STG_META.T_PRIMARY_KEYS PK -- WHERE COLUMN_NAME = 'CANDIDATE_ID'
 ...
 WHERE MK.PK_COLUMNS = 1 
   AND MK.NEW_COLUMN_NAME = MK.NEW_TABLE_NAME_ID
   AND MK.NEW_COLUMN_NAME = PK.NEW_COLUMN_NAME
...
UPDATE MS_RAW.STG_META.SF_TABLES AS t
   SET PRIMARY_KEY_NAME     = CASE WHEN pk.PK_COLUMNS = 2
       ...
       NEW_PRIMARY_KEY_NAME = CASE WHEN pk.PK_COLUMNS = 2
                                   THEN pk.NEW_TABLE_NAME_ID
                                   ELSE pk.NEW_COLUMN_NAME
                              END,
...
WHERE NEW_PRIMARY_KEY_EXPRESSION IS NOT NULL;

-- 22 -- CREATE NEW _ID COLUMN FOR TABLES WHERE PK_COLUMNS = 2 ---------------------------------
...
WHERE NEW_COLUMN_EXPRESSION LIKE '%)::NUMERIC(24,0)';

-- 22 NEW SURROGATE KEYS
INSERT INTO  MS_RAW.STG_META.T_PRIMARY_KEYS
SELECT * REPLACE('PK-' || TABLE_NAME AS PK_CONSTRAINT_NAME
                , NEW_TABLE_NAME_ID AS COLUMN_NAME
                , NEW_TABLE_NAME_ID AS NEW_COLUMN_NAME
                , NULL AS MS_TABLE_SCHEMA
                , NULL AS MS_TABLE_NAME
                , NULL AS MS_COLUMN_NAME
                , 'PK-' || TABLE_NAME AS CONSTRAINT_NAME
                , 0 AS ORDINAL_POSITION
                , TRUE AS IS_SURROGATE_KEY
                )
FROM MS_RAW.STG_META.T_PRIMARY_KEYS T
WHERE PK_COLUMNS = 2
  AND ORDINAL_POSITION = 1
  AND NOT EXISTS (SELECT 1 FROM MS_RAW.STG_META.T_PRIMARY_KEYS AS S
                  WHERE S.TABLE_SCHEMA = T.TABLE_SCHEMA
                    AND S.TABLE_NAME   = T.TABLE_NAME
                    AND S.ORDINAL_POSITION = 0)  
ORDER BY TABLE_SCHEMA, TABLE_NAME
;


-- 305 UPDATE ALL PK COLUMNS ---------------------------
...
-- 175 _IDs THAT DO NOT EXIST AS PK AND WILL BE CORRECTED WITH THE FK
SELECT NEW_COLUMN_NAME, *
FROM MS_RAW.STG_META.SF_COLUMNS C 
WHERE NEW_COLUMN_NAME ILIKE '%_ID'
  AND NOT STARTSWITH(NEW_COLUMN_NAME,'_')
...
SELECT * FROM MS_RAW.STG_META.T_PRIMARY_KEYS WHERE NEW_TABLE_NAME       = 'JOB_APPLICATION';    -- CURRENTLY IS 'APPLICATION_ID';
